#!/usr/bin/env python3
"""
run.py - One-step helper to prepare and run the NAO simulation project

This script automates the common setup tasks for the repository so users
don't have to install each Python dependency manually. It does *not*
attempt to install system-level packages like ROS 2 or Webots; those must
be installed beforehand (see docs/README.md).
...
Note about environment sourcing: this script spawns a bash subprocess and
sources `install/setup.bash` where needed. It cannot change the caller's
shell environment; to use the workspace interactively you still need to
source `install/setup.bash` in your terminal after a successful build.
...
def pip_install(packages):
    ...
    if not pip.exists():
        raise RuntimeError("Virtualenv not found. Run with --setup first.")
...
def generate_embeddings():
    gen_script = ROOT / 'my_nao_controller' / 'generate_action_embeddings.py'
    ...
    python = VENV_DIR / 'bin' / 'python' if VENV_DIR.exists() else PYTHON
    ...
    if generated.exists():
        dest = ROOT / 'my_nao_controller' / 'action_embeddings.pkl'
...
def launch_simulation():
    ...
    cmd = "bash -lc 'source install/setup.bash && ros2 launch my_nao_controller robot_launch.py'"
    run(cmd, shell=True)
...
def run_brain():
    if not check_cmd('ros2'):
        print('ros2 command not found. Source your ROS 2 installation or install ROS 2.')
        return
    cmd = "bash -lc 'source install/setup.bash && ros2 run my_nao_controller nao_brain'"
    run(cmd, shell=True)
...
def run_test_actions():
    if not check_cmd('ros2'):
        print('ros2 command not found. Source your ROS 2 installation or install ROS 2.')
        return
    cmd = "bash -lc 'source install/setup.bash && ros2 run my_nao_controller test_actions'"
    run(cmd, shell=True)


def parse_args():
    p = argparse.ArgumentParser(description='One-step helper for my_nao_controller')
    ...
    p.add_argument('--run-brain', action='store_true', help='Run the nao_brain node')
